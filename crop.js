...
function createPreview(imageSrc, file, cropSize) {
  ...
  img.onload = () => {
    ...
    // Calculate aspect ratio of the crop dimensions
    const targetAspectRatio = cropSize.width / cropSize.height;
    const imageAspectRatio = img.width / img.height;

    let cropWidth, cropHeight, offsetX, offsetY;

    if (imageAspectRatio > targetAspectRatio) {
      // Crop the sides for a wide image
      cropHeight = img.height;
      cropWidth = img.height * targetAspectRatio;
      offsetX = (img.width - cropWidth) / 2;
      offsetY = 0;
    } else {
      // Crop the top and bottom for a tall image
      cropWidth = img.width;
    ...
    // Draw the cropped area on a high-resolution canvas
    ctx.drawImage(
      img,
      offsetX, offsetY, cropWidth, cropHeight,                // Source crop area
